...
export class Server{
    ...
    private async createHandler(request, response) : Promise<void> {
		...
		await this.create(request.body.id,request.body.word, request.body.img,request.body.languages,request.body.definition,response);
    }

    private async viewHandler(request, response) : Promise<void> {
		await this.view(request.body.word, response);
    }
    private async uviewHandler(request, response) : Promise<void> {
		await this.viewuser(request.body.id, response);
    }
    ...
    private async getUserHandler(request, response) : Promise<void>{
        await this.getUserInfo(request.body.id, response);
    }
    ...
    //Word Functions
    public async create(id:string,word:string, img:string, languages:string,definition:string,response) : Promise<void> { //create the word
        console.log("creating word in create '" + word + "'");
        await this.dataBase.create(word, img,languages,definition);
        await this.dataBase.worduserinfo(id,word);
        response.write(JSON.stringify({'result' : 'created',
                           'word' : word }));
        response.end();
    }
    ...
    public async viewuser(id:string, response): Promise<void>{  //view word and image and languages supported for definition
        console.log('view user word : '+id);
        let info = await this.dataBase.getuserword(id);
        
        response.write(JSON.stringify(info));
        
	    response.end();
    }
    public async getUserInfo(id:string, response): Promise<void>{  //view word and image and languages supported for definition
        console.log('view user word for : '+id);
        let info = await this.dataBase.getUser(id);
        // response.write(JSON.stringify(
        //     {'result' : 'created',
        //     'username' : 'haha',
        //     'portrait' : '1',
